...
class _CreateShiftDialogState extends State<CreateShiftDialog> {
  ...
  final _shiftsState = ValueNotifier<AsyncValue<List<Shift>>>(const AsyncLoading());
  ...
  // Mock time off requests (в реальном приложении это будет из API)
  final List<String> _timeOffRequests = ['1']; // Employee ID '1' requested time off
  ...
  void _checkConflicts() {
    ...
    final conflicts = ShiftConflictChecker.checkConflicts(
      ...
      timeOffRequests: _timeOffRequests,
    );
  ...
  Future<void> _save() async {
    ...
    if (hasHardErrors) {
      locator<NotifyService>().setToastEvent(
        ToastEventError(message: 'Невозможно создать смену из-за конфликтов'),
      );
      return;
    }
    ...
    if (hasWarnings && !_ignoreWarning) {
      locator<NotifyService>().setToastEvent(
        ToastEventWarning(message: 'Подтвердите предупреждение для продолжения'),
      );
      return;
    }
                    ...
                    return SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: state.isLoading ||
                                (ShiftConflictChecker.hasHardErrors(_currentConflicts)) ||
                                (ShiftConflictChecker.hasWarnings(_currentConflicts) && !_ignoreWarning)
                            ? null
                            : _save,
