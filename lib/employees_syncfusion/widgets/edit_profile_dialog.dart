...
class _EditProfileDialogState extends State<EditProfileDialog> {
  ...
  Future<void> _initializeForm() async {
    ...
    // Инициализировать остальные поля из профиля
    _selectedPosition = widget.profile.role;
    _selectedBranch = widget.profile.branch;
    _selectedHireDate = widget.profile.hireDate;
    _email = widget.profile.email;
    _phone = widget.profile.phone;
    _avatarUrl = widget.profile.avatarUrl;

    // Получить текущий статус Employee из repository
    try {
      final employee = await _employeeRepository.getEmployeeById(
        widget.employeeId,
      );
      if (employee != null) {
        setState(() {
      ...
      setState(() {
        _selectedStatus = 'active'; // Fallback значение
        _isLoadingStatus = false;
      });
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      await widget.viewModel.updateEmployee(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_isLoadingStatus)
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                          value: 'vacation',
                          child: Text('В отпуске'),
                        ),
                        DropdownMenuItem(
                          value: 'sick_leave',
                          child: Text('На больничном'),
                        ),
                      ],
                      onChanged: (value) =>
                          setState(() => _selectedStatus = value),
                      validator: (value) =>
                          value == null ? 'Выберите статус' : null,
                    ),
                  const SizedBox(height: 16),
                  InkWell(
                  ...
                  TextFormField(
                    ...
                    // Валидация не требуется - поле опциональное
                  ),
                  const SizedBox(height: 24),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (widget.profile.workedHours > 0) ...[
                          const SizedBox(height: 4),
                          Text(
                            'Текущая зарплата: ${(_currentRate * widget.profile.workedHours).toStringAsFixed(2)} ₽',
